...
env = UnityEnvironment(file_name="build3/Knights of Papers.exe", seed=1, side_channels=[])
print("env connected")
...
class DualCritic(nn.Module):
    def __init__(self, state_size, action_size, visual_input_shape):
        ...
        self.conv3 = nn.Conv2d(1, 10, kernel_size=3, stride=2)
        self.conv4 = nn.Conv2d(10, 20, kernel_size=3, stride=2)

        self.fc4 = nn.Linear(self.conv_output_size + state_size + action_size, 128)
...
class ReplayBuffer:
    ...
    def sample(self, batch_size):
        ...
        for i in ind:
            ...
            vision.append(np.array(v, copy=False))
            actions.append(np.array(a, copy=False))
            next_states.append(np.array(ns, copy=False))
            next_vision.append(np.array(nv, copy=False))
...
model = SAC(actuator_obs_space, visual_obs_space, max_action, action_space)
actor_losses = []
...
for episode in range(num_episodes):
    ...
    while len(decision_steps_wooden) > 0 and len(decision_steps_grass) > 0:
        ...
        for agent_id in decision_steps_wooden.agent_id:
            ...
            with torch.no_grad():
                action = np.expand_dims(model.select_action(visual_obs, obs).numpy()[0] * max_action, axis=0)
            ...
            next_visual_obs = torch.tensor(decision_steps_wooden.obs[1][agent_id], dtype= torch.float32)
        ...
        for agent_id in decision_steps_grass.agent_id:  
            ...
            next_visual_obs = torch.tensor(decision_steps_wooden.obs[1][agent_id - 6], dtype= torch.float32)
            done = len(terminal_steps_wooden) > 0 or len(terminal_steps_grass) > 0
            experience_replay.add((obs, visual_obs, action[0], next_obs, next_visual_obs, reward, done))
...
torch.save(model, folder + model_name + ".pth")
torch.save({
