...
env = UnityEnvironment(file_name="environment-builds/linux/gpubuild.x86_64", seed=1, side_channels=[], no_graphics=True)
print("env connected")
...
class Actor(nn.Module):
    ...
    def _get_conv_output_size(self, shape):
        x = torch.rand(1, 3, *shape).to(self.conv1.weight.device)
...
class SAC:
    def __init__(self, state_size, visual_input_shape, max_action, action_size):
        ...
        # self.actor = Actor(state_size, action_size, max_action, visual_input_shape).to(device)
        # self.dualcritics = DualCritic(state_size, action_size, visual_input_shape).to(device)
        # self.actor_loss = 0
        self.actor = Actor(state_size, action_size, max_action, visual_input_shape).to(self.device)
        self.dualcritics = DualCritic(state_size, action_size, visual_input_shape).to(self.device)
        self.actor_loss = 0
        self.target_critics = DualCritic(state_size, action_size, visual_input_shape).to(self.device)

        # self.target_critics = DualCritic(state_size, action_size, visual_input_shape).to(device)
    ...
    def select_action(self, visual_input, vector_input):
        ...
        visual_input = visual_input.unsqueeze(0) if visual_input.dim() == 3 else visual_input
        vector_input = vector_input.unsqueeze(0) if vector_input.dim() == 1 else vector_input
        
        visual_input = visual_input.to(self.device)
        vector_input = vector_input.to(self.device)
...
for episode in range(num_episodes):
    ...
    while len(decision_steps_wooden) > 0 and len(decision_steps_grass) > 0:
        ...
        for agent_id in decision_steps_wooden.agent_id:
            obs = torch.tensor(decision_steps_wooden.obs[0][agent_id], dtype=torch.float32).to(device)
            visual_obs = torch.tensor(decision_steps_wooden.obs[1][agent_id], dtype=torch.float32).permute(2, 0, 1).to(device)  # Change to (C, H, W)
            ...
            next_obs = torch.tensor(decision_steps_wooden.obs[0][agent_id], dtype=torch.float32).to(device)
            next_visual_obs = torch.tensor(decision_steps_wooden.obs[1][agent_id], dtype= torch.float32).permute(2, 0, 1).to(device)
            # experience_replay.add((obs, visual_obs, action[0], next_obs, next_visual_obs, reward, done))
            experience_replay.add((obs.cpu(), visual_obs.cpu(), action[0], next_obs.cpu(), next_visual_obs.cpu(), reward, done))

        for agent_id in decision_steps_grass.agent_id:  
            obs = torch.tensor(decision_steps_grass.obs[1][agent_id - 6], dtype=torch.float32).to(device)
            visual_obs = torch.tensor(decision_steps_grass.obs[0][agent_id - 6], dtype= torch.float32).permute(2, 0, 1).to(device)
            ...
            next_obs = torch.tensor(decision_steps_wooden.obs[0][agent_id - 6], dtype=torch.float32).to(device)
            next_visual_obs = torch.tensor(decision_steps_wooden.obs[1][agent_id - 6], dtype= torch.float32).permute(2, 0, 1).to(device)
            done = len(terminal_steps_wooden) > 0 or len(terminal_steps_grass) > 0
            # experience_replay.add((obs, visual_obs, action[0], next_obs, next_visual_obs, reward, done))
            experience_replay.add((obs.cpu(), visual_obs.cpu(), action[0], next_obs.cpu(), next_visual_obs.cpu(), reward, done))
    ...
    print(model.actor_loss)
    print(model.actor_loss.item())
