...
env = UnityEnvironment(file_name="build8.app", seed=1, side_channels=[], no_graphics=False)
env.reset()
...
model = SAC(vector_obs_size, visual_obs_shape, max_action, action_size)
checkpoint = torch.load('saved_gpu_models/saved_models/sac_model20240809-043056.pth', map_location=torch.device('cpu'))
